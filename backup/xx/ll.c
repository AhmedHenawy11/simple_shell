...
void add_to_history(const char *command) {
    // If history is at maximum capacity, shift elements to make room for the new command
...
int main(void) {
    ...
    while (1) {
        ...
        // Check if the user entered 'history' to display command history
        if (strcmp(user_input_line, "history\n") == 0) {
            display_history();
        }
